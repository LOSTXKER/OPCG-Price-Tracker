...
- งาน UI ที่เปลี่ยนรูปร่าง/ตำแหน่งของที่คนเห็นและมีทางให้เลือก → หน้าลอง `/proto/<slug>` ให้เจ้าของงานเคาะก่อน ห้ามแตะของจริงจนกว่าเคาะ (ยกเว้น prop `variant` ค่าเริ่มต้น = ของเดิม) · โครง/กติกา = skill `proto` (ระดับเครื่อง `~/.claude/skills/proto/` · โครงใน repo: `src/app/proto/`) · ก่อนส่งลิงก์ต้องผ่าน `proto-shots` และแนบภาพรวม (ด่าน proto-sheet-gate ตรวจ)
...
- `DESIGN.md` (ถ้ามี) = ฉบับบาง ≤4KB: ตัวตน · ground truth ชี้ไฟล์ · "ตอนนี้ใช้อะไร" · บทเรียนเทคนิค — ไม่ใช่สมุดคำห้าม · ตอน dev เปิด `/proto` ได้โดยไม่ login (ยกเว้นไว้ใน proxy)
