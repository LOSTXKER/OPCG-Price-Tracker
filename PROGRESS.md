...
## ✅ ปรับตาม feedback เบสรอบล่าสุด (2026-07-17 บ่าย)

- **ตัดชั้น "ขยับแรงวันนี้" ทิ้ง** (เบสสั่ง) — ลบ watchlist-mover-shelf.tsx + selectWatchlistMovers + เทสต์
- **ถอดระบบปักหมุดออกทั้งหน้า** (เบสสั่ง) — ไม่มี pin glyph/ปุ่ม/ตัวกรอง "ปักหมุดเท่านั้น"/pinned-first อีกแล้ว · เรียงเริ่มต้น = เพิ่มล่าสุดก่อน · **API/DB field `pinnedAt` ยังอยู่** (ไม่แตะ schema — ถ้าจะถอดฝั่งหลังบ้านต้องรอบแยก)
- **Header/แท็บจัดใหม่**: CTA "เพิ่มการ์ด/สร้างแจ้งเตือน" เป็นปุ่ม primary สีทองขนาดเต็ม (ตามแท็บ) · แท็บตัวหนังสือล้วน (ตัดไอคอน) สูงขึ้น · เพิ่มช่องว่างทุกชั้น (header mb-4/5, content pt-4/5, ช่องระหว่าง section space-y-4/5)

## ✅ ปรับตาม feedback รอบ 6 (2026-07-17)

- ช่องค้นหาเป็น input เห็นตลอด (เลิกยุบเป็นไอคอน) ทั้งสอง breakpoint
- ปุ่ม "แก้ไข" → **"เลือก"** (ไอคอน ListChecks, คีย์ใหม่ `watchlistSelectMode` 3 ภาษา) · แถบโหมดเลือกเรียบขึ้น: เลือกแล้ว N · เลือกทั้งหมด · ลบ(N) · ยกเลิก (ตัด "ล้างที่เลือก")
- ตัด "·▲5·▼3" ออก · จำนวนย้ายไปเป็น **ตัวเลขบนแท็บ** ("การ์ดที่ติดตาม 8" — แท็บแจ้งเตือนก็มี)
- ตัดราคาเยนออกจากตาราง desktop (เหลือบาทเดียว) · period pill ย้ายไปหัวรายการมือถือ (แถวเดียวกับปุ่มเรียง)

## ✅ ตัวกรองเกม + เลือกเกมก่อนใน picker + อัปเกรดแท็บแจ้งเตือน (2026-07-17)

- **แถวกรองเกมโผล่ตลอด** ทั้งแท็บการ์ดที่ติดตามและแจ้งเตือนราคา (ทุกเกม · One Piece · Pokémon เร็วๆนี้ — เหมือนพอร์ต) — ถอด gate `length > 1` สองจุด
- **ตัวเลือกการ์ด game-first**: refactor ที่ `CardPickerForm`/`SelectStep` จุดเดียว ครอบทุก picker (watchlist add · alert create · portfolio add · compare · marketplace) — แถวเลือกเกมอยู่บนสุด (เกม live เลือกได้ / เกม comingSoon เป็น pill disabled) → ส่ง `game` เข้า /api/sets + fetchCards (หลังบ้านรองรับอยู่แล้ว) และล้าง set/facet เมื่อสลับเกม
- **แท็บแจ้งเตือน**: เรียงตามความใกล้เป้า (ทะลุเป้าอยู่บนสุด · ไม่มีราคาอยู่ล่างสุด — pure fn + test 9 ข้อใน alerts-sort.ts) · ช่องค้นหาโผล่เมื่อมี >3 รายการ · สร้างแจ้งเตือนเติมราคาปัจจุบันให้อัตโนมัติ (unify กับ flow กระดิ่ง = ปิด TRACK-04)
- ไอเดียที่เสนอแล้วยังไม่ทำ (รอเบสเคาะ): แถบ progress ราคา→เป้าต่อแถว · ยุบส่วนประวัติ · แจ้งเตือนเด้งซ้ำ (re-arm, ฟีเจอร์ใหญ่) · เส้นเป้าบนกราฟหน้าการ์ด
