...
## 🔨 รอบล่าสุด (ยังไม่ commit) — ตัดทางลัด/ค้นหาออกจากเมนู + ซ่อมสัดส่วน SegmentedControl

- **ลบกริด "ทางลัด" ออกจาก `/more` ทั้งบล็อก** และ **ตัดค้นหาออกจากเมนู** (เบสสั่ง) — ค้นหาเหลือทางเข้าเดียวคือไอคอนในหัวเว็บมือถือ (ทุกหน้ายกเว้นหน้าแรกที่ hero เป็นเจ้าของ) · ตรวจ SSR แล้ว `/more` ไม่มี `ทางลัด`/`grid-cols-3` และไม่มีแถวค้นหา (ที่เหลือคือช่องค้นหาใน header desktop + ลิงก์ footer ซึ่งโชว์แค่ `md` ขึ้นไป)
- **คืนแถวที่เคยถูกตัดเพราะซ้ำกับไทล์** เพื่อไม่ให้ปลายทางหาย: เรียกดู = มาแรง · เปรียบเทียบการ์ด · เด็คและเครื่องมือ · บัญชีของฉัน ได้ จัดการแจ้งเตือนราคา คืน · บล็อกท้ายได้ คู่มือ คืน (ชุดการ์ด ยังไม่ใส่ เพราะเป็นแท็บล่าง)
- **ซ่อมปุ่มเลือกมุมมองที่ดูไม่สมส่วน — ต้นเหตุอยู่ใน `SegmentedControl` ไม่ใช่ปุ่มนั้น**: บนมือถือ track (`before:inset-y-1`) กับ thumb (`before:inset-y-1`) inset เท่ากันเป๊ะ ขอบทั้งสองจึงทับกันพร้อมรัศมีมุมไม่เท่ากัน (12px vs 9.6px) → เห็นเป็นมุมซ้อนเบี้ยว. แก้ให้ thumb ซ้อน **ใน** track: `before:inset-y-1.5 before:inset-x-0.5` (เหลือขอบ track 2px รอบ thumb) และถอด override `before:inset-x-1` / `[&_button]:before:inset-x-1` ใน `ViewModeControl` ที่เป็นตัวทำให้ขอบชนกัน
- **ตรวจส่วนอื่นตามที่สั่ง**: ของเดิมพลาดเหมือนกันหมดเพราะใช้ component เดียวกัน — วัดที่ 390px หลังแก้: เกรด (251×44) · มุมมอง (88×44) · ช่วงเปลี่ยนแปลง (168×44) ทั้งสามได้ track 36px / thumb 32px / ขอบ 2px เท่ากันหมด

**Verify**: 128 files / 700 tests ผ่าน (อัปเดต `view-mode-control.test.tsx` ที่ pin คลาส inset เก่าไว้) · `tsc` ผ่าน · eslint 0 issue · วัดจาก DOM ที่ 390px · restart dev server อีกครั้ง (serve `more-client.tsx` เก่าค้าง)

## 🔨 รอบก่อน (ยังไม่ commit) — ลบ More sheet ทำ "ดูเพิ่มเติม" เป็นแท็บจริง

- **ลบ `src/components/layout/more-sheet.tsx` ทิ้ง** · `bottom-nav.tsx` เลิกมี `TabButton`/`moreOpen`/`MoreSheet` → แท็บที่ 5 เป็น `TabLink href="/more"` ธรรมดา และยังสว่างเป็น fallback ของทุกหน้าที่ 4 แท็บไม่ได้ถือ (`moreActive = !tabActive.some(Boolean)`)
- **ย้ายกริดทางลัด 6 ช่องขึ้นไปเป็นบล็อกแรกของ `/more`** (หัวข้อ "ทางลัด") — ชุดเดียวคงที่ทั้ง guest/member: ค้นหา · มาแรง · เด็ค · เปรียบเทียบการ์ด · คู่มือ · แจ้งเตือนราคา
- **กันซ้ำในหน้าเดียว**: ลบ section "เรียกดู" ทั้งบล็อก (ทั้ง 4 แถวกลายเป็นไทล์แล้ว) · เอาแถว แจ้งเตือนราคา ออกจาก "บัญชีของฉัน" · เอาแถว คู่มือ ออกจากบล็อกท้าย → ตรวจแล้วเหลือ href ซ้ำแค่ `/settings` (การ์ดบัญชีด้านบน + แถวตั้งค่า ซึ่งเป็นเจตนา)
- Honey ยังเป็นแถวใน "บัญชีของฉัน" (ถือจุดแดง pending และเป็นสมอของ section เวลาปิด marketplace)

**Verify**: 128 files / 700 tests ผ่าน · `tsc` ผ่าน · eslint 0 issue · เปิด `/more` จริง: ไทล์ 6 ช่องสูงเท่ากัน 94px, ไม่มี dialog, แท็บที่ 5 เป็นลิงก์และ `aria-current="page"` · **ยังไม่ได้ดูบนจอมือถือจริง** (iframe ค้างที่ Suspense fallback ของ client page — ข้อจำกัด harness ไม่ใช่บั๊กหน้า: SSR ส่ง markup ครบ)
