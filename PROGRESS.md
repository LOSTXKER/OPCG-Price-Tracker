...
อัปเดตล่าสุด: 2026-07-25 — **Card Picker ไม่มีเลขขั้น + ไอคอนเกม/ชุดตรงกัน + ตัวเลือกเกมออกจากแถบแท็บพอร์ต + ซ่อมเส้นขั้น + รื้อแถวรายการพอร์ตบนมือถือให้อ่านได้**
...
1. ให้เบสรีเฟรช Watchlist/Portfolio แล้วลอง “เพิ่มการ์ด” เพื่อรีวิว filter workspace แบบไม่มีเลข + คู่ไอคอนเกม/ชุดที่จับให้ตรงกันแล้ว
1b. ค้าง (รอเบสตัดสิน): ทั้งเว็บยังมี "เครื่องหมายเกม" 3 แบบ — จุดสีใน header `GameSwitcher` · `Layers3` ใน game scope ของ MINE toolbar · `Gamepad2` ใน card picker; ถ้าจะยึด "1 concept 1 icon" ต้องเลือกอันเดียวแล้วยุบทั้ง 3 จุด (คนละไฟล์ คนละ surface — งานแยก)
2. ถ้าหน้าตาผ่าน ค่อย commit/push งานชุดนี้; รอบนี้ยังไม่ได้ commit
3. ก่อนเปิด Pokémon จริง ให้ audit/guard launch-ready card ที่ API read/write boundary แยกงาน
4. แนะนำอัปเกรด Vercel CLI จาก 55.0.0 เป็น 57.0.0 ด้วย `npm i -g vercel@latest`
