...
- 🏗️ **Portfolio** — เดิมรื้อเป็นหน้าเดียวแบบ Robinhood แต่ **เบส veto (2026-07-17): เก็บ 2 แท็บแบบเดิม** → revert โครงหน้าแล้ว (การเก็บ 2 แท็บ = ข้อยกเว้นโดยเจตนาเจ้าของ ต่อ VISION §5.3 — อย่ารื้อซ้ำ) · ที่คงไว้คือการแก้ระดับชิ้นส่วน: PriceTag มีลูกศรทุก delta ในตาราง, Switch กลางใน dialog แก้ไข, ปุ่มลบมองเห็นบนมือถือ, หัวตารางเลิกมุดใต้ header, ลิงก์มูฟเวอร์เสีย
- 🏗️ **รื้อ toolbar + ตาราง Watchlist** — ตัดเหลือเฉพาะที่จำเป็น (feedback เบส): ค้นหา(ยุบ) · เรียง · ตัวกรอง · มุมมอง · แก้ไข + จำนวน · **ชุดการ์ดย้ายเข้า popup ตัวกรอง** · ปุ่มช่วงเวลาบน desktop เหลือเฉพาะมุมมองกริด · ตารางจอใหญ่โชว์ 24H/7D/30D พร้อมกันแบบ CoinMarketCap กดหัวคอลัมน์เรียงได้ · ป้ายปักหมุด/กระดิ่งเลิกบังรูปการ์ด
...
- 🏠 **Home มือถือ: sort dropdown → แถบแบบรายการโปรด** (2026-07-17) — period pill 24h/7d/30d + tap-sort ราคา·เปลี่ยนแปลง แทน ToolbarSortDropdown เดิม · **home.5 ปิดแล้ว** (% ใน MobileCardItem ตาม period ไม่ hardcode 24h) · sync สองทาง (กดหัวคอลัมน์ change บน desktop → pill ตาม · เปลี่ยน pill → re-sort ตาม) ผ่าน `periodForColumn`/`retargetSortForPeriod` (market-types + test) · โหมด PSA 10 ซ่อน pill+change sort (แถวไม่มี % ให้ดู) · rarity sort เหลือ desktop เท่านั้น (เจตนา — กรอง rarity อยู่ใน FilterModal) · ⚠️ home.4 หลักฐานบรรทัดเดิมตกยุค (dropdown ถูกลบ) แต่ประเด็น grid ไม่มีปุ่ม sort ยังเปิดอยู่ · /search มือถือยังมี mismatch % คงที่ 24h vs sort 7d/30d (ยังไม่แตะ — นอกขอบเขตรอบนี้)
