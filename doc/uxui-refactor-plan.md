...
> **อัปเดต 2026-07-16:** มี audit รอบสอง 319 ข้อ (ข้อใหม่ + สถานะปัจจุบันของข้อเดิม) ที่ **[uxui-audit-findings-2026-07-16.md](uxui-audit-findings-2026-07-16.md)** — ก่อนทำ Phase ไหนให้เปิดเทียบทั้ง 2 ไฟล์ · Watchlist ถูกรื้อใหม่แล้ว (PR #116) ข้อเดิมที่ชี้หน้านี้อาจ obsolete · ⚠️ **Portfolio เก็บ 2 แท็บตามคำสั่งเบส 2026-07-17 — ข้อยกเว้นโดยเจตนาต่อ VISION §5.3 ห้ามรื้อเป็นหน้าเดียวซ้ำ**
...
- [x] แตก **`useRecentSearches` เสร็จ** (`hooks/use-recent-searches.ts` · recent localStorage+state+push/remove/clear/refresh) → migrate ทั้ง 3 (card-search·command-search·hero-search) ยุบ readRecent/writeRecent/pushRecent ซ้ำ · dedup unify เป็น case-insensitive
- [x] **engine เดียวครบ (branch `feat/phase2.4-search-engine`)** — 3 ชิ้นกลาง: `hooks/use-search-keyboard-nav.ts` (activeIdx + ↑↓/Enter/Esc, `arrowUpFloor` 0=card-search / -1=palette+hero) · `shared/search-result-row.tsx` (เนื้อในแถว thumb+ชื่อ+set+rarity+ราคา, props คุม divergence, default=หน้าตา dropdown เดิม) · `useCardSearch` +AbortController +`error` +`keepPreviousOnError` (additive, deck-calc/marketplace ไม่กระทบ) · **command-search + hero เลิก fetch เอง → engine กลาง** · `SearchResultsDropdown` ใช้ `SearchResultRow` ภายใน · **คงหน้าตา+พฤติกรรมทุก surface เป๊ะ** (hero UPPERCASE/blur/font-price vs palette/dropdown ตัวเล็ก) · verify: tsc0/lint0/test56/build✓ + **live browser 3 surface** (hero/palette/add-dialog, arrow-nav+Enter dispatch ผ่าน, 0 console error) + adversarial review 5 มิติ (2 confirmed low = per-fetch `setActiveIdx(-1)` reset ตกหล่นตอน migrate → แก้แล้วด้วย effect reset-on-results ทั้ง 2 surface)
- [ ] ระหว่างทางเก็บด้วย: `HOME-09`/`DISCOVERY-08` debounce จริง 250-300ms (ตอนนี้คง 0ms ตาม behavior เดิม command/hero) · `HOME-08` **ARIA combobox → เลื่อน Phase 5.0/`RESPONSIVE-01`** (2.x = dedup ล้วน ไม่เพิ่ม capability · `SearchResultRow` กลางทำให้เติม role=option/aria ที่เดียวจบทีหลัง) · `DISCOVERY-09` ผลลัพธ์ 3 ทางเข้าเป็นชุดเดียว (การ์ด+เซ็ต+หน้า+ค้นรูป)
...
- [x] `IDENTITY-01` + `IDENTITY-10` **auth kit เสร็จ (branch `feat/phase2.5-auth-kit`)** — 5 component `components/auth/` + `lib/auth/password-rules.ts`: `AuthShell` (hero slot: 2-col login/register + gradient/checklist ต่าง · 1-col forgot/reset) · `OAuthButtons` (Google/FB byte-identical + owns signInOAuth) · `PasswordInput` (`leftIcon`/`showToggle`/`hint`/`labelClassName` · **IDENTITY-10 a11y fix: ปุ่มตา focusable + aria-label/aria-pressed** ที่เดียวจบ 5 จุด) · `PasswordRules` (+`getPasswordRules` single source, ยุบ PASSWORD_RULES ซ้ำ 3) · `FormError` · migrate login/register/forgot/reset + `section-security` · **คงหน้าตา+schema/supabase/redirect เป๊ะ** (IDENTITY-09 validation นอก scope · section-security label คง `text-eyebrow` ผ่าน prop) · verify tsc0/lint0/test56/build✓ + eyeball register(2-col)/forgot(1-col) + **ปุ่มตา focus ring ยืนยัน a11y** + adversarial review 5 surface **0 confirmed**
- [x] `CONTENT-02` **guide kit เสร็จ (branch `feat/phase2.5-guide-kit`)** — 4 component กลาง `components/guide/`: `GuideSourceList` (`internal` prop Link/anchor) · `GuideCallout` (`tone`, body=children คงมาร์กอัปเดิม) · `GuidePrevNext` · `CardThumbStrip` (`size` sm/md/lg/xl) · migrate 6 หน้าลูก · **พ่วง visible ที่เบสอนุมัติ**: `CONTENT-07` ทุกหน้าใช้ `PageHeader` (large-title มือถือ) + `CONTENT-08` หน้า `buying` restyle เข้าชุด (✓/✗→lucide Check/X · read-price→GuideCallout · number badge→primary tint · footer→GuidePrevNext · h1 i18n) · verify tsc0/lint0/test56/build✓ + eyeball buying+getting-started + adversarial review 6 หน้า **0 confirmed** (5 canonical behavior-preserving · buying 2 nit คงเดิมให้เบส eyeball: callout heading amber, prev label "Sets" ยังไม่ i18n) · **ยังไม่ทำ**: `CONTENT-09` ลดสี callout (เบสไม่เลือก) · `CONTENT-03` perf query/ISR (แยก)
- [x] `SETS-04` **SetPosterTile เสร็จ (branch `feat/phase2.5-sets-trending`)** — `components/sets/set-poster-tile.tsx` (presentational server-safe) ยุบ tile ซ้ำ (sets index + other-sets rail) · `showCount` prop toggle บรรทัดจำนวน · **แก้ sizes drift** (grid 2 ที่เหมือนกันเป๊ะ → unified `31vw/23vw/15vw/13vw` แก้ over(45vw)+under(12vw)-fetch, invisible) · verify tsc0/lint0/test56/build✓ + eyeball /sets grid
- [~] `DISCOVERY-10` trending ใช้ MarketTable — **⛔ ทำ behavior-preserving ไม่ได้ (scout verdict)**: `MarketTableRow`/`MobileCardItem` hardcode หลายอย่างต่างจาก trending (CardImageButton เปิด modal · rarity แยกคอลัมน์ · delta size ต่าง · **mobile row แสดง period switch 7d/30d + views ไม่ได้เลย**) → จะทำต้องแก้ shared ที่ home+search ใช้ (เสี่ยง) หรือยอมหน้าตา MarketTable = **redesign ต้องเบส sign-off** → **reclassify: เป็นงาน redesign/MarketTable-extension แยก ไม่ใช่ 2.5 dedup**
- [x] `HONEY-03` **single source + แก้ bug เลข (branch `feat/phase2.5-honey`)** — `lib/honey/streak.ts` (standalone client-safe, ไม่ดึง prisma เข้า bundle) export `STREAK_TIERS`/`getStreakReward`/`getStreakTier`/`getStreakMultiplier` · **แก้ live bug: UI โชว์ 10/20/30 เก่า (drift หลัง CHECKIN 10→5 commit c094778) → FREE จริง 5/10/15** (เบสเคาะโชว์ base FREE) · migrate 5 ไฟล์ (streak-tier-indicator·honey-sidebar·streak-card·streak-info-popover·honey-mock-preview) · verify tsc0/lint0/**test56 (policy test ผ่าน = server index.ts ไม่แตะ)**/build✓ + eyeball /honey (+5 ปัจจุบัน · +10 tier 7วัน) · **ยังไม่ทำ**: popover component 3→1 consolidation (แค่ยุบ data ให้เลขตรง, structural merge เลื่อน) · `HONEY-07` mock rewrite (eyeball-limited เลื่อน)
- [~] `ADMIN-06` **เสร็จ (branch `feat/phase2.5-admin-form`)** — `lib/admin/use-admin-form.ts` hook เดียว owns form state/dirty/error/saving(useTransition)/submit lifecycle (validate→adminFetch POST·PATCH·PUT→toast→router.push+refresh) + คุม create-vs-edit body ต่างกันผ่าน `toBody(f,isEdit)` + ยุบ getElementById→requestSubmit hack เข้า `submitFromBar` (AdminSaveBar API เดิม ไม่แตะ กัน array-editor พัง) · migrate **7 ฟอร์ม** (achievement=reference · blog=raw fetch→adminFetch+errorToast:false เก็บ showPreview/slugify local · shop·event·bonus·schedule·template) · **defer** `raffle` (572 บรรทัด nested prize array + cloneFrom) + array-editor (rank-tiers) + card-editor/card-edit-form (bespoke) · verify tsc0/lint0/test56/build✓ + adversarial review 7 ฟอร์ม+hook = **0 reachable behavior change** (blog error-fallback ต่างเฉพาะ branch ที่ route ไม่มีทางคืน — blog routes = adminApiHandler คืน `{error}` เสมอ) · ⚠️ login-gated → เบส eyeball เพิ่มได้: `/admin/blog/new` · `/admin/honey/{achievements,shop,events,missions/{bonus,schedule,templates}}/new`
- [~] `ADMIN-02` (+`ADMIN-01`, `RESPONSIVE-03` ในตัว) **migrate 5 list เข้า `AdminDataTable` เสร็จ (branch `feat/phase2.5-admin-table`)** — `bonus-list`(reference) · `schedule-list` · `templates-list` · `preview-client` · `honey-shop-manager` (ทั้ง 5 ไม่มี mobile fallback → ได้ built-in list fallback ฟรี = RESPONSIVE-03) · faithful: cell content/class copy เป๊ะ · header align ผ่าน `headerClassName` (cn=tailwind-merge override text-left) · cell align ผ่าน `className` · คง AdminEmptyState conditional เดิม (swap แค่ populated table) · คง handler (toggle/delete confirmDialog/crud.remove loading) · verify tsc0/lint0err/test56/build✓ + adversarial review 5 ตาราง = **0 finding** (img warning = pre-existing) · ⚠️ login-gated eyeball: `/admin/honey/{shop,missions/bonus,missions/schedule,missions/templates,missions/preview}` · **defer**: `blog/page`(server comp + มี mobile fallback แล้ว → ต้องแตก client comp กัน RSC function-prop, value ต่ำ) · `honey-transaction-list`(infinite Load-More) · `set-row`(inline-edit) · `yuyutei/snkrdunk-match`(matching UI) · `card-editor`(embedded table)
...
- [~] `KIT-09` **partial เสร็จ (branch `feat/phase2.6-kit-09`)** — ย้าย feature-bound widget ที่ชัด+ไม่ conflict: `compare-button` → `components/compare/` · `notification-bell` → `components/layout/` (importer 2 ตัวอยู่ layout header ทั้งคู่) ด้วย `git mv` + update importers (@/ absolute) · **คงใน shared (cross-feature จริง)**: `rank-icon` (honey+admin+layout · +conflict กับ #79 honey rename), `upgrade-badge`/`upgrade-dialog` (16 importers ข้ามฟีเจอร์ = shared ตามกฎ 3 ชั้น), `watchlist-star` · **`streak-tier-indicator` = orphan (0 importer)** → flag ลบ (permission เบส) ไม่ย้ายของตาย · verify tsc0/lint0/test56/build✓
- [x] `IDENTITY-11` **เสร็จ (#77)** — ย้าย **13 ไฟล์ settings** (8 `section-*` + 5 `account-*`) จาก `components/profile` → `components/settings/` (ใหม่) ด้วย `git mv` (history preserved) · shared/public อยู่ต่อใน profile: `public/`(25) + `profile-{types,data-context,seller-card,share-menu,achievements}` · rewrite import: `./profile-types`→`@/components/profile/profile-types`, `./public/profile-cover`→absolute (10+1 จุด) · sibling `./account-*` คงrelative (ย้ายพร้อมกัน) · update 10 settings pages importers · verify tsc0/lint0/test56/build✓ (pure structural move, static-verified = no eyeball)
- [x] `KIT-04` **เสร็จ (#78)** rename local `ListRow`→`WatchlistRow` (watchlist) กันชื่อชน + scope docstring `ui/list-row` (grouped/settings เท่านั้น) + แก้ dead ref `card-table.tsx`→`admin-data-table.tsx` ใน AGENTS.md · `KIT-06` **เสร็จ (branch `feat/phase2.6-kit-06`)** size prop → `sm|md|lg` (6 comp เปลี่ยน "default"→"md" + data-attr selector avatar/select · zero caller churn · behavior เหมือนเดิม) · `HONEY-05` **เสร็จ (branch `feat/phase2.6-honey-05`)** แตก `honey-sidebar`(494)→`honey-status-bar`+`stat-card`+`rank-progress`+`guide-contents` (verbatim, review 100% faithful)
