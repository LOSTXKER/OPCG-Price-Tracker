...
export function HoneyUsersManager() {
  ...
  const handleGrant = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setGrantMsg(`Granted! New balance: ${data.honeyPoints}`);
        setGrantAmount("");
        setGrantReason("");
        setGrantUserId(null);
        loadUsers();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Honey Users</h1>
      ...
      {/* Grant Modal */}
      {grantUserId && (
        <div className="panel space-y-3 border-amber-200 dark:border-amber-500/30 p-4">
          <div className="flex items-center justify-between">
            <h3 className="flex items-center gap-2 font-semibold">
              <Gift className="h-4 w-4 text-amber-600 dark:text-amber-400" /> Grant Honey
            </h3>
            <button onClick={() => { setGrantUserId(null); setGrantMsg(null); }}>
          ...
          <div className="flex items-center gap-2">
            <Button onClick={handleGrant} disabled={granting || !grantAmount || !grantReason} size="sm" className="gap-1.5">
              <Send className="h-3.5 w-3.5" /> {granting ? "Granting..." : "Grant"}
            </Button>
            {grantMsg && <span className="text-sm text-amber-600 dark:text-amber-400">{grantMsg}</span>}
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/50 bg-muted/30">
                  ...
                  <th className="px-4 py-2.5 text-right font-medium text-muted-foreground">Honey</th>
                  <th className="px-4 py-2.5 text-right font-medium text-muted-foreground">Streak</th>
                  <th className="px-4 py-2.5 text-right font-medium text-muted-foreground">Tier</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <button
                        ...
                        className="rounded p-1.5 text-amber-600 dark:text-amber-400 transition-colors hover:bg-amber-100 dark:bg-amber-500/10"
                        title="Grant Honey"
                      >
        ...
        <div className="lg:col-span-2">
          ...
          {!selectedUser && !detailLoading && (
            <div className="panel flex flex-col items-center gap-2 p-12 text-center text-muted-foreground">
