...
export default function ProtoCardDetail() {
  ...
  return (
    <div className="pb-24">
      ...
      {/* ZONE 1 — identity */}
      <section className="glow-honey px-5 pb-2 pt-3">
        <div className="mx-auto w-[58%] max-w-[230px]">
          <FoilCard />
        </div>
      ...
      <section className="px-5 pt-5 text-center">
        ...
        <div key={tier.key + edition} className="rise mt-1 flex items-end justify-center gap-3">
          <span className="tnum text-[42px] font-extrabold leading-none text-foreground">{tier.display}</span>
        </div>
        ...
        {/* 3-stat market summary */}
        <div className="mt-4 grid grid-cols-3 divide-x rounded-2xl surface-1 hairline py-3" style={{ "--tw-divide-opacity": 1 } as React.CSSProperties}>
          {[
            { l: "Last Sale", v: tier.display },
            { l: "Lowest Ask", v: `฿${(tier.price + 800).toLocaleString()}` },
            { l: "Sales 30d", v: "1,204" },
          ].map((s) => (
            <div key={s.l} className="px-2" style={{ borderColor: "var(--p-hair)" }}>
      ...
      {/* ZONE 3 — grade chip rail (re-prices the page) */}
      <section className="mt-5">
        <div className="no-sb flex gap-2 overflow-x-auto px-5 pb-1">
          {tiers.map((t) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "ease-chrome flex shrink-0 flex-col items-start rounded-xl px-3.5 py-2 text-left",
          ...
          {([
            ...
            <button
              ...
              className={cn(
                ...
                tab === k ? "text-foreground surface-2 hairline" : "text-muted-foreground",
              )}
            >
              {label}
            </button>
      ...
      {/* sticky CTA */}
      <div className="frost sticky bottom-0 z-20 mt-6 flex items-center gap-3 px-5 py-3" style={{ boxShadow: "inset 0 1px 0 0 var(--p-hair)" }}>
        <button
...
function SourceBadge({ s }: { s: string }) {
  ...
  return (
    <span className="rounded px-1.5 py-0.5 text-[10px] font-bold" style={{ background: "var(--p-s2)", color }}>
      {s}
    </span>
  )
}
    ...
    <div className="space-y-2">
      {LISTINGS.map((l, i) => (
        <div key={i} className="flex items-center gap-3 rounded-2xl surface-1 hairline p-3">
          <div className="flex size-9 items-center justify-center rounded-lg surface-2 text-xs font-bold" style={{ color: "var(--primary)" }}>
...
function Meta() {
  return (
    <div className="rounded-2xl surface-1 hairline p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-lg font-extrabold text-foreground">
            Tier <span style={{ color: "var(--primary)" }}>S</span> · Leader
          </p>
        </div>
        <div className="text-right">
      ...
      <p className="mt-3 border-t pt-3 text-xs text-muted-foreground" style={{ borderColor: "var(--p-hair)" }}>
        จาก 1,840 เด็คใน 32 ทัวร์ · อัปเดต 6 ชม.ที่แล้ว
      </p>
    </div>
  )
}
