...
> = {
  ...
  speaks: {
    name: "D · ปุ่มตัวกรองพูดชื่อชุด — ฉันแนะนำอันนี้",
    summary:
      "โครงเหมือน A เป๊ะทุกอย่างตามที่เบสสั่ง (ชุดอยู่ในตัวกรอง · เกรดซ้ายของตัวกรอง · มุมมองขวาของแคปซูลเรียง) — ต่างกันที่ **ปุ่มตัวกรองเปลี่ยนคำเป็นชื่อชุดที่กำลังดูอยู่** พอเลือก op13 ปุ่มจะอ่านว่า “OP13” แทนคำว่า “ตัวกรอง” ยังมีไอคอนกับเลขบอกว่ามีตัวกรองอื่นเปิดอยู่กี่อันเหมือนเดิม",
    tradeoff:
      "ได้ทั้งความกระชับของ A และการมองเห็นชุดแบบเดิม โดย **ไม่เพิ่มแถวเหมือน B และไม่กินที่เพิ่มเลย** — คำว่า “OP13” สั้นกว่าคำว่า “ตัวกรอง” ด้วยซ้ำ เกรดจึงได้ที่เท่ากับ A · ข้อแลกที่เหลือ: ปุ่มเปลี่ยนคำไปมา คนที่จำว่า “ปุ่มตัวกรองอยู่ตรงนี้” อาจสะดุดครั้งแรกที่เห็นมันกลายเป็น OP13 (แก้ด้วยไอคอนที่ไม่เปลี่ยน) · และการเปลี่ยนชุดยังเป็น 3 จังหวะเหมือน A กับ B",
    rows: "2 แถว",
  },
};
...
const NOTES = [
  ...
  "กดปุ่ม “เลือกชุด op13 แล้ว” ข้างบนเพื่อดูว่าแต่ละแบบบอกหรือไม่บอกว่ากำลังดูชุดไหนอยู่ — นี่คือจุดเดียวที่ A · B · D ต่างกัน (A ไม่บอก · B บอกด้วยป้ายเพิ่มมาอีกแถว · D บอกบนปุ่มตัวกรองโดยไม่เพิ่มแถว)",
  ...
  "บนคอมไม่เปลี่ยนอะไรเลยทุกแบบ — แถบบนคอมเป็นแถวเดียวอยู่แล้วและมีที่พอ รอบนี้แตะเฉพาะมือถือ",
] as const;
...
export default function MobileToolbarProtoPage() {
  const [way, setWay] = useProtoVariant<Way>("v", WAY_VALUES, "speaks");
  ...
  /* แบบ D — ปุ่มเดียวกัน แต่พูดชื่อชุดที่กำลังดูอยู่แทนคำว่า "ตัวกรอง"
     ไอคอนกับเลขคงเดิม ปุ่มจึงยังอ่านออกว่ากดแล้วเจอตัวกรอง */
  const filterButtonSpeaking = (
    <FilterButton
      aria-label={setChosen ? "ตัวกรอง · กำลังดูชุด OP13" : "ตัวกรอง"}
      active={filterCount > 0}
      count={filterCount}
      className="shrink-0"
    >
      {setChosen ? "OP13" : "ตัวกรอง"}
    </FilterButton>
  );
    ...
    <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-10">
      <div className="mx-auto max-w-[1200px]">
        ...
        <p className="mt-2 max-w-3xl text-body text-muted-foreground">
          ...
          ทำเหมือนกันหมดในแบบ A · B · D แล้ว
        </p>
        ...
        <section className="mt-6 grid gap-8 lg:grid-cols-[375px_minmax(0,1fr)]">
          <div className="min-w-0">
            ...
            <div className="-mx-4 sm:mx-0">
              <div className="hairline w-[375px] max-w-full overflow-hidden bg-background sm:rounded-2xl">
                ...
                {(way === "asked" || way === "chip" || way === "speaks") && (
                  <>
                    <div className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="min-w-0 flex-1">{gradeControl}</div>
                        <div className="shrink-0">
                          {way === "speaks" ? filterButtonSpeaking : filterButton}
                        </div>
                      </div>
                      {way === "chip" && setChosen && (
                        <div className="pt-2">
          ...
          <div className="space-y-6">
            ...
            <div className="hairline rounded-2xl bg-card p-4">
              <p className="text-eyebrow mb-2">ทางที่ลองแล้วตกไป</p>
              <p className="text-body-sm text-muted-foreground">
                <strong className="text-foreground">
                  ยุบเหลือแถวเดียว
                </strong>{" "}
                (แคปซูลเรียง + ตัวกรอง + มุมมอง อยู่แถวเดียวกัน แล้วเอาเกรดเข้ากล่อง
                ตัวกรองไปด้วย) — ทำแล้ววัดจริง{" "}
                <strong className="text-foreground">ยัดไม่ลงที่จอ 375px</strong>{" "}
                แคปซูลเรียงโดนตัดจนปุ่ม 7d หายทั้งปุ่ม · และเกรดคือของที่คนสลับบ่อย
                ที่สุดในหน้านี้ (Raw ↔ PSA 10) ซ่อนเข้ากล่องแล้วหน้าจอจะไม่บอกเลยว่า
                กำลังดูราคาเกรดไหนอยู่ ซึ่งอันตรายกว่าไม่บอกชุด เพราะตัวเลขราคา
                หน้าตาเหมือนกันหมด
              </p>
            </div>
            ...
            <div className="hairline rounded-2xl bg-card p-4">
              ...
              <p className="mt-2 text-body-sm text-muted-foreground">
                ...
                · แบบ D มีไว้เพื่อไม่ต้องเลือกข้าง: ชุดเข้าไปอยู่ในกล่องตามที่สั่ง
                แต่ชื่อชุดยังอ่านได้จากหน้าจอตลอดเวลาผ่านปุ่มตัวกรอง
                · ถ้าเคาะ A ฉันจะไปแก้กติกาในคู่มือให้ตรงกันด้วย ไม่งั้นครั้งหน้า
                จะมีคนย้ายกลับ
              </p>
            </div>
